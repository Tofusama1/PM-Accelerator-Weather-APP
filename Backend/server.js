...
// CORS Configuration - FIXED: Added missing headers
app.use(
  cors({
    origin: [
...
const validateWeatherData = (req, res, next) => {
  ...
  if (end > new Date(now.getTime() + 14 * 24 * 60 * 60 * 1000)) {
    return res
      .status(400)
      .json({ error: "End date cannot be more than 14 days in the future" });
  }
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    if (
      !username ||
      !validator.isLength(username.trim(), { min: 3, max: 50 })
    ) {
...
const getLocationDetails = async (location) => {
  try {
    ...
    if (!API_KEY) {
      console.warn(
        "OpenWeather API key not configured, using basic location data"
      );
      return {
        city: location,
        state: "",
    ...
    if (geoResponse.data && geoResponse.data.length > 0) {
      ...
      return {
        ...
        formatted_address: `${locationData.name}${
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log("Creating weather record for:", {
        location,
        startDate,
        endDate,
        userId,
...
app.get("/api/export/:format", authenticateToken, async (req, res) => {
  try {
    ...
    if (records.length === 0) {
      ...
      return res.status(404).json({ error: "No weather records found" });
    }
    ...
    console.log("First record structure:", {
      ...
      weatherDataType: typeof records[0].weather_data,
      weatherDataLength: records[0].weather_data
        ? JSON.stringify(records[0].weather_data).length
        : 0,
    });

    // Helper function to safely parse weather data
    const parseWeatherData = (weatherData) => {
      try {
        if (typeof weatherData === "string") {
          const parsed = JSON.parse(weatherData);
          return parsed.weather_data || [];
        }
    ...
    switch (format.toLowerCase()) {
      case "json": {
        ...
        const flattenedJSON = records.flatMap((record) => {
          const weatherArray = parseWeatherData(record.weather_data);
          console.log(
            `Record ${record.id} has ${weatherArray.length} weather entries`
          );

          return weatherArray.map((entry) => ({
            record_id: record.id,
        ...
        console.log("Flattened JSON entries:", flattenedJSON.length);
        console.log("Sample entry:", flattenedJSON[0]);
      ...
      case "csv": {
        ...
        const csvHeaders =
          "Record ID,User ID,Location,Start Date,End Date,Date,Time,Temperature,Feels Like,Humidity,Pressure,Wind Speed,Wind Direction,Weather Main,Weather Description,Weather Icon,Visibility,Clouds,Created At\n";
          ...
          .flatMap((record) => {
            ...
            return weatherArray.map((entry) => {
              ...
              return [
                ...
                escapeCSV(entry.weather_main || ""),
                escapeCSV(entry.weather_description || ""),
                entry.weather_icon || "",
                entry.visibility || "",
                entry.clouds || "",
                record.created_at,
      ...
      case "xml": {
        console.log("Processing XML export...");
        ...
        const xmlEntries = records
          .flatMap((record) => {
            const weatherArray = parseWeatherData(record.weather_data);

            return weatherArray.map(
              (entry) => `    <entry>
      <recordId>${record.id}</recordId>
      <userId>${record.user_id}</userId>
      <location>${escapeXML(record.location)}</location>
      <startDate>${record.start_date}</startDate>
      <endDate>${record.end_date}</endDate>
      <date>${entry.date || ""}</date>
      <time>${entry.time || ""}</time>
      <temperature>${entry.temperature || ""}</temperature>
      <feelsLike>${entry.feels_like || ""}</feelsLike>
      <humidity>${entry.humidity || ""}</humidity>
      <pressure>${entry.pressure || ""}</pressure>
      <windSpeed>${entry.wind_speed || ""}</windSpeed>
      <windDirection>${entry.wind_direction || ""}</windDirection>
      <weatherMain>${escapeXML(entry.weather_main || "")}</weatherMain>
      <weatherDescription>${escapeXML(
        entry.weather_description || ""
      )}</weatherDescription>
      <weatherIcon>${entry.weather_icon || ""}</weatherIcon>
      <visibility>${entry.visibility || ""}</visibility>
      <clouds>${entry.clouds || ""}</clouds>
...
${xmlEntries}
        ...
        res.setHeader(
          "Content-Disposition",
          "attachment; filename=weather_data.xml"
        );
      ...
      default:
        ...
        return res.status(400).json({
          error: "Unsupported export format. Supported formats: json, csv, xml",
